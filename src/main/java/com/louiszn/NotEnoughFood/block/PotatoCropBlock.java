...
public class PotatoCropBlock extends TemplateCropBlock {
    public PotatoCropBlock(Identifier identifier) {
        super(identifier, 0);
        ...
        if (!world.isRemote) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                ItemEntity item = new ItemEntity(
                        ...
                        new ItemStack(
                                switch (meta / 2) {
                                    case 0, 3 -> ItemListener.potato;
                                    default -> ItemListener.poisonousPotato;
                                }
                        )
                );
            ...
            if (meta == 7 && world.random.nextFloat() <= 0.2F) { // 20% chance
                float offsetX = world.random.nextFloat() * offsetRange + (1.0F - offsetRange) * 0.5F;
                float offsetY = world.random.nextFloat() * offsetRange + (1.0F - offsetRange) * 0.5F;
                float offsetZ = world.random.nextFloat() * offsetRange + (1.0F - offsetRange) * 0.5F;

                ItemEntity poisonDrop = new ItemEntity(
                        world,
                        (float) x + offsetX,
                        (float) y + offsetY,
                        (float) z + offsetZ,
                        new ItemStack(ItemListener.poisonousPotato)
                );
                poisonDrop.pickupDelay = 10;
                world.spawnEntity(poisonDrop);
            }
    ...
    public int getTexture(int side, int meta) {
        return TextureListener.potatoCropTextures[Math.min(meta * 4 / 8, 3)];
    }
}
